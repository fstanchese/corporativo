SELECT DISTINCT 
    GRA.ID AS COD_HIST_ESCOLAR_EXT,
    MAT.WPESSOA_ID || NVL(CURR2.CURSO_MIG_ID,
                ...
                END || NVL(COF2.PERIODO_ID,
                           COF.PERIODO_ID) || NVL(COF2.CAMPUS_ID,
                                                  COF.CAMPUS_ID) AS COD_ALUNO_EXT,
       NVL(PLE2.ID,
                    PLE.ID) || CUR.CURSONIVEL_ID AS COD_PERIODO_LETIVO_EXT,
       DIS.COD_DISCIPLINA_EXT AS COD_DISCIPLINA_EXT,
       CASE 
          WHEN CDC.NOTATI_ID IN (12300000000001, 12300000000004) AND substr(GRA.CriAval_Id,-4)=0004 THEN GRA.N8
          WHEN CDC.NOTATI_ID IN (12300000000001, 12300000000004) AND substr(GRA.CriAval_Id,-4)=2001 THEN GRA.N5
          WHEN CDC.NOTATI_ID IN (12300000000001, 12300000000004) AND substr(GRA.CriAval_Id,-4)=1999 THEN GRA.N7   
          WHEN CDC.NOTATI_ID IN (12300000000001, 12300000000004) AND substr(GRA.CriAval_Id,-4)=1997 THEN GRA.N7    
          WHEN CDC.NOTATI_ID IN (12300000000001, 12300000000004) AND substr(GRA.CriAval_Id,-4)=1993 THEN GRA.N7        
          WHEN CDC.NOTATI_ID IN (12300000000001, 12300000000004) AND substr(GRA.CriAval_Id,-4)=1991 THEN GRA.N7            
          WHEN CDC.NOTATI_ID IN (12300000000001, 12300000000004) AND substr(GRA.CriAval_Id,-4)=1990 THEN GRA.N7      
          WHEN CDC.NOTATI_ID IN (12300000000001, 12300000000004) AND substr(GRA.CriAval_Id,-4)=1989 THEN GRA.N9         
          WHEN CDC.NOTATI_ID IN (12300000000001, 12300000000004) AND substr(GRA.CriAval_Id,-4)=0001 THEN GRA.N11    
          WHEN CDC.NOTATI_ID IS NULL AND CDC.SEMPROVA ='on' and sta.nome like '%Aprovado%' then '10'
          else 
            '0'
        end AS NOTA_ALUNO,
        CASE WHEN STA.NOME  LIKE '%Falta%' then 3000000003007
             else
               STA.ID
        end COD_CONCEITO_EXT,

        CASE WHEN STA.NOME  LIKE '%Falta%' then 'Reprovado por Frequencia'
             else
               STA.NOME 
        end DSC_CONCEITO,

        CASE WHEN STA.NOME  LIKE '%Falta%' then 'F'
             else
               SUBSTR(STA.NICK,1,1)
        end SGL_CONCEITO,

        CASE WHEN STA.NOME  LIKE '%Falta%' then 'N'
             WHEN  SUBSTR(STA.NICK,1,1) = 'A' THEN 'S'
             ELSE 'N'
        end IND_APROVACAO,
       NULL AS COD_DISC_DISP_EXT,
       NULL AS COD_USUARIO_EXT,
       NULL AS DAT_CADASTRO,
       NULL AS COD_INSTIT_EXTERNA_EXT,
       NULL AS NOM_DISC_DISP,
       NULL AS CARGA_HORARIA_DISP,
       NULL AS NUM_CREDITOS,
       NULL AS NOTA_ALUNO_DISP        
 FROM GRADALU GRA
  JOIN TURMAOFE TOF
 ...
 WHERE STA.ID IN (3000000003006, 3000000003004, 3000000003008,3000000003005,3000000003007) 
