<?php
	...
	echo $view->JS("
			...
			$('.openColorBoxSenha').colorbox({
				...
				height:'450px',
			 	escKey:false,
    			onClosed:function(){
					location.href='casenha_igeracao.php'
				}
			});
	...
	if($_POST[p_O_Option] == 'insert')
	{
		...
		echo $view->Js("$.colorbox({href:'casenha_isenha.php?casenha_id="._UrlEncrypt($casenha_id)."',width:'500px', height:'450px',iframe: true,escKey:false,onClosed:function(){location.href='casenha_igeracao.php'}});");
		...
		$form = new Form();
			...
			$form->Fieldset($caEvento->Recognize($_COOKIE["p_CAEvento_Id"],"RecReduz"));
				...
				$form->Input("Nome",'autocomplete',	array("execute"=>"CAWPesDet.AutoCompleteAluno","name"=>'p_WPessoa_Id', "class"=>"size70", "required"=>'1',"label"=>""));
