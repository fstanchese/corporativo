<?php
	...
	$user = new User ();
	$app = new App("Candidatos que Compareceram e Não Geraram Processo - Controle de Atendimento","Candidatos que Compareceram e Não Geraram Processo - Controle de Atendimento",array('ADM','CPD','CASENHAGER'),$user);
	...
	else
	{
		...
		if ($_POST["consultar"] == "Gerar em PDF")
		{
			...
			$vDescricao = 'Candidatos Que Não Geraram Processo - Evento:  ' . $dadosEvento[RECOGNIZE];
		
			$dbData->Get("select distinct(WPessoa.Nome),WPessoa.CPF,CAEvXWPes.Id as CAEvXWPes_Id from WPessoa,CASenha,CASenhaRegra,CASenhaTi,CAAssunto,CAEvXWPes
							where
                  			...
                  			and
                     			WPessoa.Id not in (
									...
									from 
										LoteFluxo,CAEvXWPes 
									where
      									LoteFluxo.CAEvXWPes_Id = CAEvXWPes.Id
      								and 
      									CAEvXWPes.CAEvento_Id in (select id from caEvento where descricao='".$dadosEvento["DESCRICAO"]."')                     
                     				)
                    		and
        						CAEvXWPes.WPessoa_Id = CASenha.WPessoa_Id 
					      	and
        						CAEvXWPes.CAEvento_Id in (select id from caEvento where descricao='".$dadosEvento["DESCRICAO"]."')					
			             	and
