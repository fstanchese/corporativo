...
and
  (
    trunc(matric.data) <= p_O_Data1 
    or
    ( trunc(matric.data) <= p_O_Data1 and trunc(matric.dtstate) > p_O_Data1 )
  )
and
  ( vest.wpleito_id >= nvl( p_WPleito_PROUNI_Id , 0) and vest.wpleito_id <= nvl( p_WPleito_PROUNI_Id ,0 ) )
and
