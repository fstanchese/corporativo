			...
			if($cont >0){
				...
				if(eregi(";;;;;;;",$buffer)){
					...
					if(trim($aluno[8]) == "Pré-Selecionado em 1ª Chamada")
					{
						...
						$wpessoa = $dbData->Row($dbData->Get("SELECT id FROM wpessoa WHERE cpf= '".trim($aluno[4])."'"));
						...
						if($wpessoa[ID] != "")
						{
							...
							//$dbData->Set("INSERT INTO wpessoa (nome, cpf) VALUES ('".trim($aluno[2])."','".trim($aluno[4])."')");
							
							//echo "INSERT INTO wpessoa (nome, cpf) VALUES ('".trim($aluno[2])."','".trim($aluno[4])."') <br>";
							...
							$dbData->Set("INSERT INTO caevxwpes (wpessoa_id, caevento_id) VALUES (".$wpessoa[ID].",199700000000030)");
